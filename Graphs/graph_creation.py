...
class TitleDictionary:
    ...
    def _create_title_dict(self):
        title_dict = {}
        unique_person_values = self.df['nconst'].unique()
        #print(unique_person_values)
        for person in unique_person_values:
            list_of_movies = self.df[self.df['nconst'] == person]['primaryTitle'].tolist()
            title_dict[person] = list_of_movies
    ...
    def _create_profession_dict(self):
        ...
        unique_person_values = self.df['nconst'].unique()
        for person in unique_person_values:
            selected_person = self.df[self.df['nconst'] == person].iloc[0]
            profession_dict[person] = selected_person['primaryName']+'_a' if selected_person['primaryProfession'] == 'actor' else selected_person['primaryName']+'_d'
...
class MovieNetwork:
    ...
    def create_graph(self):
        unique_person_values = list(self.name_movie_dict.keys())
        #print(unique_person_values)
        for person in unique_person_values:
            self.add_node(person)
        for i, node1 in enumerate(unique_person_values):
            for node2 in unique_person_values[i+1:]:
